...
export function groupSearchResults(results: DiscoverySearchResult[]): GroupedDiscoveryResults[] {
  ...
  return DISCOVERY_TYPE_ORDER.filter((type) => grouped.has(type)).map((type) => ({
    type,
...
export function findDocumentBySlug(
  ...
  return documents.find((document) => document.type === type && document.slug === slug)
}
