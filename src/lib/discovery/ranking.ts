...
function scoreKeywordMatch(document: DiscoveryDocument, terms: string[]): number {
  if (terms.length === 0) {
    return 0
  }
...
function scoreFieldMatch(values: string[], terms: string[], weight: number): number {
...
export function scoreDiscoveryDocument(
  ...
  return (
    ...
    scoreFieldMatch(document.technologies, terms, SEARCH_RANKING_WEIGHTS.technology) +
    scoreFieldMatch(document.expertise, terms, SEARCH_RANKING_WEIGHTS.expertise) +
