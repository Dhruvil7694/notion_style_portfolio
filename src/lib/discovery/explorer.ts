...
import type { KnowledgeGraphPayload } from "@/lib/knowledge/types"
...
function documentsById(documents: DiscoveryDocument[]): Map<string, DiscoveryDocument> {
...
function linkedContentIds(graph: KnowledgeGraphPayload, entityId: string): string[] {
...
export function resolveEntityNavigation(
  ...
  const relatedExpertiseSlugs = Array.isArray(conceptEntity?.metadata?.relatedExpertiseSlugs)
    ? conceptEntity.metadata.relatedExpertiseSlugs.filter(
        (item): item is string => typeof item === "string"
      )
    : []

  const relatedExpertiseIds = relatedExpertiseSlugs.map((item) => `expertise:${item}`)

  const linkedExpertiseIds = coLinkedEntityIds(graph, entityId, "uses_expertise")
  const techIds = coLinkedEntityIds(graph, entityId, "uses_technology")

  return {
    relatedExpertise: pickDocuments([...relatedExpertiseIds, ...linkedExpertiseIds], lookup),
    relatedTechnologies: pickDocuments(techIds, lookup),
...
export function buildTopicClusters(
  ...
  return anchors.map((anchor) => buildClusterFromAnchor(graph, documents, anchor))
}

export function getPopularTopics(documents: DiscoveryDocument[], limit = 8): DiscoveryDocument[] {
  return [...documents]
...
export function getRecentlyUpdated(documents: DiscoveryDocument[], limit = 8): DiscoveryDocument[] {
  return [...documents]
    .sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt))
    .slice(0, limit)
}

export function getFeaturedProjects(documents: DiscoveryDocument[], limit = 4): DiscoveryDocument[] {
  return documents
...
export function getFeaturedConcepts(documents: DiscoveryDocument[], limit = 4): DiscoveryDocument[] {
  return documents
...
export function getFeaturedExpertise(documents: DiscoveryDocument[], limit = 4): DiscoveryDocument[] {
  return documents
...
export function getExplorerSections(documents: DiscoveryDocument[]) {
  return {
    ...
    technologies: documents.filter((document) => document.type === "technology"),
