...
import type { DiscoveryDocument, DiscoveryDocumentType, DiscoveryIndexPayload } from "./types"

function mapEntityType(type: KnowledgeEntity["type"]): DiscoveryDocumentType | null {
  if (type === "writing") {
    return "article"
  }
  ...
  const featured =
    entity.type === "expertise"
      ...
      : entity.type === "technology"
        ? graph.technologies.find((item) => item.slug === entity.slug)?.registered
        : entity.type === "concept"
...
function entityToDiscoveryDocument(
  ...
  const updatedAt = entity.updatedAt ?? entity.publishedAt ?? new Date(0).toISOString()
  ...
  return {
    ...
    slug: entity.slug,
    url: entity.url,
    keywords: [...new Set([entity.title, ...keywords, ...tags, ...concepts, ...technologies, ...expertise])],
    expertise,
    technologies,
    concepts,
...
async function buildDiscoveryIndexUncached(siteUrl: string): Promise<DiscoveryIndexPayload> {
...
export async function getDiscoveryDocuments(siteUrl: string): Promise<DiscoveryDocument[]> {
