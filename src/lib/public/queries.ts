...
import type { Content, Education, Experience, Project, Resume, Skill } from "@/types/database.helpers"
...
async function fetchPublicSettings(): Promise<PublicSettings> {
  ...
  const { data } = await supabase
    .from("settings")
    .select("key, value")
    .in("key", ["site_settings", "social_links", "contact_info", "about_content"])
...
async function fetchAllPublishedProjects() {
  ...
  const query = supabase
    .from("projects")
    .select(
      "id, slug, title, summary, tagline, category, role, challenge, solution, approach, impact, content, tech_stack, technologies, concepts, expertise_slugs, ai_summary, featured, status, published_at, updated_at, github_url, live_url, icon_name"
    )
    .eq("status", "published")
  ...
  if (error && /challenge|solution|impact|display_order/i.test(error.message)) {
    const fallback = await supabase
      .from("projects")
      .select(
        "id, slug, title, summary, tagline, category, role, content, tech_stack, technologies, concepts, expertise_slugs, ai_summary, featured, status, published_at, updated_at, github_url, live_url, icon_name"
      )
      .eq("status", "published")
      .order("featured", { ascending: false })
...
export async function getPublishedExpertiseAreas(options?: { featured?: boolean }) {
