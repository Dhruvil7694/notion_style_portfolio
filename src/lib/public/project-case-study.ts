...
import { parseTradeoffItems, type ProjectTradeoffV2 } from "@/lib/knowledge/schemas"
...
export function flattenTechStackGroups(groups: ProjectTechStackGroup[]): string[] {
...
export function buildProjectDetailMetadataLine(project: {
  ...
  return [monthYear, project.category, status, project.role].filter(Boolean).join(" · ")
}
