import { getISTSecond } from "@/lib/public/workspace-utils"
...
export function pickWeightedMessage(messages: ContextMessage[], seed: number): string {
...
export function pickDeterministicMessages(
  ...
  for (let salt = 0; salt < messages.length + count && results.length < count; salt += 1) {
