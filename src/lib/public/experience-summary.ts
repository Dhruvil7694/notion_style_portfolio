...
import { isVolunteerExperience } from "@/lib/public/experience-duration"
...
type ExperienceRangeFields = Pick<Experience, "start_date" | "end_date" | "role">
...
function getCurrentExperience(experiences: Experience[]): Experience | null {
  ...
  return (
    [...experiences].sort(
      (left, right) =>
        parseISO(right.start_date).getTime() - parseISO(left.start_date).getTime()
    )[0] ?? null
  )
}
...
export function getCurrentCompanyMonths(experiences: Experience[]): number | null {
  const current = getCurrentExperience(experiences)
  if (!current) {
    return null
  }
