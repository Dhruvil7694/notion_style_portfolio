...
import { createCopilotChatModel } from "@/lib/ai/models"
import { buildCopilotContextPrompt } from "@/lib/ai/prompts"
import type { CopilotIntent, CopilotPendingAction } from "@/lib/ai/types"
import type { CopilotToolName } from "@/lib/copilot/tools"
import { buildCopilotToolDescriptions, executeCopilotTool } from "@/lib/copilot/tools"
...
const ABOUT_FIELD_HINTS: Array<{ field: string; keywords: RegExp }> = [
  { field: "intro_tools", keywords: /\b(tools\s+line|stack\s+line|toolchain)\b/i },
  { field: "career_intro", keywords: /\b(career|1point1|umbrella)\b/i },
  { field: "after_umbrella", keywords: /\b(after\s+umbrella|production\s+ai|evals|guardrails)\b/i },
  { field: "retrieval", keywords: /\b(retrieval|rag\s+paragraph|chunk)\b/i },
  { field: "ownership", keywords: /\b(ownership|end\s+to\s+end|over-?engineer)\b/i },
  { field: "outside", keywords: /\b(outside\s+work|hobby|photography|music|papers)\b/i },
  { field: "mcp", keywords: /\b(mcp\s+paragraph|tool[-\s]calling\s+paragraph)\b/i },
]
...
function looksLikeAboutEdit(message: string): boolean {
  return /\b(about\s+(me|section|page|content|paragraph|tagline|intro))\b/i.test(message)
}
...
export async function selectToolsNode(
  ...
  if (state.intent === "relationships") {
    const techMatch = message.match(/(?:technology|skill|concept)\s+([a-z0-9-]+)/i)
    if (techMatch) {
      toolCalls.push({
        id: crypto.randomUUID(),
...
function describeAction(name: string): { label: string; description: string } {
  switch (name) {
    ...
    case "applyFaq":
      return { label: "Apply FAQ", description: "Save the generated FAQ to the project." }
    case "applySummary":
      return { label: "Apply summary", description: "Save the generated AI summary." }
    case "applyTakeaways":
      return { label: "Apply takeaways", description: "Save the key takeaways." }
    case "applyTradeoffs":
      return { label: "Apply tradeoffs", description: "Save the tradeoffs." }
    case "applyTechnologies":
      return { label: "Apply technologies", description: "Update technology links." }
    case "applyExpertise":
      return { label: "Apply expertise", description: "Update expertise links." }
    case "applyConcepts":
      return { label: "Apply concepts", description: "Update concept links." }
    case "applyRelationships":
      return { label: "Apply relationships", description: "Update relationship links." }
    case "createProject":
      return { label: "Create project", description: "Insert the new project as draft." }
    case "updateProject":
      return { label: "Update project", description: "Write the proposed project changes." }
    case "createSkill":
      return { label: "Create skill", description: "Add the new skill to the taxonomy." }
    case "createTechnology":
      return { label: "Create technology", description: "Register the technology page." }
    case "createConcept":
      return { label: "Create concept", description: "Register the concept page." }
    default:
...
export async function executeToolsNode(
  ...
  for (const toolCall of state.toolCalls) {
    ...
    if (toolCall.requiresConfirmation || result.requiresConfirmation) {
      ...
      const preview =
        (result.data && typeof result.data === "object" && "preview" in (result.data as Record<string, unknown>)
          ? (result.data as { preview: unknown }).preview
          : result.preview) ?? null

      let pendingArgs = toolCall.args
      if (toolCall.name === "updateAboutSection" && preview && typeof preview === "object") {
        const p = preview as {
          field?: string
...
export async function validateNode(
  ...
  if (failed.length > 0) {
    return {
      error: failed.map((f) => f.error).filter(Boolean).join("; ") || "Tool validation failed",
    }
  }
  return {}
}
...
export async function respondNode(
  ...
  if (state.intent === "audit" && state.toolResults.some((r) => r.name === "auditPortfolio")) {
    const auditData = state.toolResults.find((r) => r.name === "auditPortfolio")?.data as
      | { score?: number; portfolioIssues?: { message: string }[] }
      | undefined

    if (auditData?.portfolioIssues) {
      const issues = auditData.portfolioIssues.slice(0, 10).map((i) => `- ${i.message}`).join("\n")
