...
import { parseFaqItems, parseProjectFacts, parseTradeoffItems } from "@/lib/knowledge/schemas"
import type { Project } from "@/types/database.helpers"

import { buildHealthCheck, computeHealthScore, getMissingLabels } from "./health-score"
...
export function auditProject(project: ProjectAuditRow): ProjectHealthReport {
  const checks = [
    ...
    buildHealthCheck("facts", Object.keys(parseProjectFacts(project.project_facts)).length > 0),
    buildHealthCheck(
      "technologies",
    ...
    buildHealthCheck("tradeoffs", parseTradeoffItems(project.tradeoffs).length > 0),
