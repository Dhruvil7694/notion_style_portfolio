...
import { revalidateKnowledgeAndDiscovery, revalidatePublicContent } from "@/lib/public/revalidate-cache"
...
const CONTENT_TYPES = ["blog", "research", "automation", "publication", "note"] as const
...
const createContentSchema = z.object({
  ...
  type: z.enum(CONTENT_TYPES).describe(`Content type: ${CONTENT_TYPES.join(", ")}.`),
  slug: z
    .string()
    .optional()
    .describe(
  ...
  excerpt: z.string().max(500).nullable().optional().describe("Short teaser (max 500 chars)."),
  tags: z
    .array(z.string())
    .default([])
...
export const applyCreateContentTool: CopilotApplyTool<typeof createContentSchema> = {
  kind: "apply",
...
const updateContentMetaSchema = z.object({
  slug: z.string().min(1).describe("Content slug — use listContent to find it."),
...
> = {
  ...
  async build(rawArgs) {
    ...
    const { data: current, error } = await supabase
      .from("content")
      .select("id, title, type, excerpt, tags, status, ai_summary, key_takeaways, expertise_slugs")
      .eq("slug", slug)
      .maybeSingle()
    if (error) return { ok: false, error: error.message }
...
export const applyDeleteContentTool: CopilotApplyTool<typeof deleteContentSchema> = {
  kind: "apply",
  ...
  async execute(rawArgs) {
    ...
    if (!current)
      return { success: false, error: `Content "${parsed.data.slug}" not found.` }

    const { error } = await supabase.from("content").delete().eq("id", current.id)
    if (error) return { success: false, error: error.message }
