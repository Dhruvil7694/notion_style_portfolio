...
export const listExpertiseTool: CopilotReadTool<typeof listExpertiseSchema, unknown[]> = {
  kind: "read",
  name: "listExpertiseAreas",
  description: "List all expertise areas (AI Engineering, RAG, Multi-Agent Systems, etc.).",
...
const createExpertiseSchema = z.object({
  ...
  slug: z.string().optional().describe("Auto-generated from title if not provided."),
...
export const proposeCreateExpertiseTool: CopilotProposeTool<typeof createExpertiseSchema> = {
  kind: "propose",
...
export const applyCreateExpertiseTool: CopilotApplyTool<typeof createExpertiseSchema> = {
  kind: "apply",
...
const updateExpertiseSchema = z.object({
  id: z.string().uuid().describe("Expertise area ID — use listExpertiseAreas to find it."),
...
export const proposeUpdateExpertiseTool: CopilotProposeTool<typeof updateExpertiseSchema> = {
  kind: "propose",
  ...
  async build(rawArgs) {
    ...
    const { data: current, error } = await supabase
      .from("expertise_areas")
      .select("title, description, summary, status, featured, keywords, icon_name")
      .eq("id", id)
      .maybeSingle()
    if (error) return { ok: false, error: error.message }
    ...
    const fields = Object.entries(changes)
      ...
      .map(([key, value]) => ({ name: key, key, before: (current as Record<string, unknown>)[key], after: value }))
...
export const applyUpdateExpertiseTool: CopilotApplyTool<typeof updateExpertiseSchema> = {
  kind: "apply",
  ...
  async execute(rawArgs) {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle()
    if (error) return { success: false, error: error.message }
    if (!data) return { success: false, error: `Expertise area ${id} not found.` }
    revalidateKG()
    return {
      success: true,
      data: { message: `Updated expertise area "${data.title}".`, redirectUrl: `/admin/expertise/${id}`, redirectLabel: "Open in editor" },
    }
  },
}
...
export const listTechnologiesTool: CopilotReadTool<typeof listTechSchema, unknown[]> = {
  kind: "read",
  name: "listTechnologies",
  description: "List all technology registry entries (LangGraph, FastAPI, etc.).",
...
const createTechSchema = z.object({
  ...
  description: z.string().optional(),
  summary: z.string().optional(),
  category: z.string().optional().describe("E.g. framework, database, cloud, ai_ml."),
...
export const proposeCreateTechnologyTool: CopilotProposeTool<typeof createTechSchema> = {
  kind: "propose",
...
export const applyCreateTechnologyTool: CopilotApplyTool<typeof createTechSchema> = {
  kind: "apply",
...
export const listConceptsTool: CopilotReadTool<typeof listConceptsSchema, unknown[]> = {
  kind: "read",
  name: "listConcepts",
  description: "List all concept registry entries (Hybrid Retrieval, Agent Memory, etc.).",
...
export const proposeCreateConceptTool: CopilotProposeTool<typeof createConceptSchema> = {
  kind: "propose",
...
export const applyCreateConceptTool: CopilotApplyTool<typeof createConceptSchema> = {
  kind: "apply",
