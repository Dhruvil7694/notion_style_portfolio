    ...
    if (field === "faq") {
      ...
      if (items.length === 0) {
        return {
          ok: false,
          error:
            "FAQ must be a non-empty array of {question, answer} objects.",
        }
      }
    }
...
export const applyCreateProjectTool: CopilotApplyTool<typeof createProjectSchema> = {
  kind: "apply",
...
export const applyArchiveProjectTool: CopilotApplyTool<typeof archiveProjectSchema> = {
  kind: "apply",
