...
import { skillFormSchema } from "@/lib/admin/schemas"
import {
...
const proposeUpdateSkillSchema = z.object({
  ...
  category: z
    .enum(["language", "framework", "tool", "cloud", "ai_ml", "soft", "other"])
    .optional(),
  proficiency: z.enum(["learning", "proficient", "expert"]).nullable().optional(),
...
export const proposeDeleteSkillTool: CopilotProposeTool<typeof deleteSkillSchema> = {
  kind: "propose",
  ...
  async build(rawArgs) {
    ...
    const { data: current, error } = await supabase
      ...
      .maybeSingle()
    if (error) return { ok: false, error: error.message }
    if (!current) return { ok: false, error: `Skill ${parsed.data.id} not found.` }
    return {
      ok: true,
      pending: {
...
export const applyDeleteSkillTool: CopilotApplyTool<typeof deleteSkillSchema> = {
  ...
  async execute(rawArgs) {
    ...
    return {
      success: true,
      data: { message: "Skill deleted.", redirectUrl: "/admin/skills", redirectLabel: "Open skills list" },
    }
  },
}
