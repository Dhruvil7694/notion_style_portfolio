...
const createEduSchema = z.object({
  institution: z.string().min(1).describe("University, bootcamp, or institution name."),
...
export const applyCreateEducationTool: CopilotApplyTool<typeof createEduSchema> = {
  kind: "apply",
...
> = {
  ...
  async build(rawArgs) {
    ...
    const { data: current, error } = await supabase
      ...
      .maybeSingle()
    if (error) return { ok: false, error: error.message }
    if (!current) return { ok: false, error: `Education entry ${id} not found.` }
...
export const applyUpdateEducationTool: CopilotApplyTool<typeof updateEduSchema> = {
  kind: "apply",
  ...
  async build(rawArgs) {
    ...
    return {
      ...
      pending: {
        ...
        fields: [
          { name: "Institution", before: current.institution, after: "(deleted)" },
...
export const applyDeleteEducationTool: CopilotApplyTool<typeof deleteEduSchema> = {
  kind: "apply",
