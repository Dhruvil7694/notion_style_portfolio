...
export const applyCreateExperienceTool: CopilotApplyTool<typeof createExpSchema> = {
  kind: "apply",
...
> = {
  ...
  async build(rawArgs) {
    ...
    const { data: current, error } = await supabase
      ...
      .maybeSingle()
    if (error) return { ok: false, error: error.message }
    if (!current) return { ok: false, error: `Experience entry ${id} not found.` }
...
export const applyUpdateExperienceTool: CopilotApplyTool<typeof updateExpSchema> = {
  kind: "apply",
...
export const applyDeleteExperienceTool: CopilotApplyTool<typeof deleteExpSchema> = {
  kind: "apply",
