...
export async function applyProjectField(
  ...
  switch (action) {
    ...
    case "applyTechnologies":
      ...
      break
    case "applyExpertise":
      updates.expertise_slugs = Array.isArray(input.content) ? input.content : []
      break
    case "applyConcepts":
  ...
  if (!data) return { success: false, error: `Project not found: ${input.slug}` }
...
export async function applyAboutSection(
  ...
  if (!confirmed) {
    return {
      success: true,
      data: {
        message: "Preview ready — confirm to apply changes to the About section.",
        preview: {
          field: input.field,
