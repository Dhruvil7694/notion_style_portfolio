...
import { auditPortfolio } from "@/lib/copilot/audit"
import { buildCopilotToolDescriptions } from "@/lib/copilot/tools"
import { buildKnowledgeGraph } from "@/lib/knowledge/graph"

const CACHE_TTL = 600

export async function getCachedPortfolioSnapshot(siteUrl: string): Promise<string> {
...
export async function getCachedKnowledgeSummary(siteUrl: string): Promise<string> {
  return unstable_cache(
    async () => {
      const graph = await buildKnowledgeGraph(siteUrl)
      if (!graph) return ""

      const topExpertise = graph.expertise.slice(0, 5).map((e) => e.title).join(", ")
      const topTech = graph.technologies.slice(0, 8).map((t) => t.name).join(", ")
      const topConcepts = graph.concepts.slice(0, 8).map((c) => c.title).join(", ")
...
export async function getCachedExpertiseSummary(siteUrl: string): Promise<string> {
  return unstable_cache(
    async () => {
      const graph = await buildKnowledgeGraph(siteUrl)
      if (!graph) return ""
      return graph.expertise
        .slice(0, 10)
        .map((area) => `- ${area.title}: ${area.description ?? area.summary ?? ""}`)
        .join("\n")
    },
...
export async function getCachedTechnologySummary(siteUrl: string): Promise<string> {
  return unstable_cache(
    async () => {
      const graph = await buildKnowledgeGraph(siteUrl)
      if (!graph) return ""
      return graph.technologies
        .slice(0, 15)
        .map((tech) => `- ${tech.name}${tech.registered ? " (registered)" : ""}`)
        .join("\n")
    },
