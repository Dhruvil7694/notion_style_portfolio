...
function resolveForRole(
  ...
  if (role === "public") {
    return { provider: settings.public_provider, modelId: settings.public_model }
  }
  if (role === "copilot") {
    return { provider: settings.copilot_provider, modelId: settings.copilot_model }
  }
  return { provider: settings.fallback_provider, modelId: settings.fallback_model }
}
...
export async function buildProviderChain(
  ...
  const orderedProviders = [
    ...
    ...DEFAULT_FAILOVER_ORDER.filter(
      (provider) => provider !== primary.provider && provider !== fallback.provider
    ),
  ]
    ...
    const modelId =
      provider === primary.provider
        ...
        : provider === fallback.provider
          ? fallback.modelId
          : PROVIDER_CONFIGS[provider].defaultModels[role === "copilot" ? "strong" : "fast"]
...
export async function resolveModelChain(role: AiRole): Promise<ProviderChainEntry[]> {
  const settings = await getAiSettings()
  return buildProviderChain(role, settings)
}

export async function resolvePrimaryModel(role: AiRole): Promise<ProviderChainEntry> {
