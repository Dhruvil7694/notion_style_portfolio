...
export async function resolveProviderKey(provider: AiProviderId): Promise<string | undefined> {
  const keys = await resolveProviderKeysMap()
  return keys[provider]
}

export async function isProviderKeyConfigured(provider: AiProviderId): Promise<boolean> {
