...
export async function decryptStoredProviderKeys(
  ...
  for (const [provider, payload] of Object.entries(encryptedKeys) as [AiProviderId, string][]) {
    try {
...
export function encryptProviderKeyUpdates(
  ...
  for (const [provider, apiKey] of Object.entries(updates) as [AiProviderId, string][]) {
