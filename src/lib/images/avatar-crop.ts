...
async function resolveImageSrc(imageSrc: string): Promise<{ src: string; revoke?: () => void }> {
...
export async function renderAvatarCropBlob(
  ...
  const outputTransform = scaleAvatarTransform(transform, editorFrameSize, outputFrameSize)
  ...
  try {
    ...
    const left = outputSize / 2 - exportDisplayWidth / 2 + clamped.offsetX * exportScale
    const top = outputSize / 2 - exportDisplayHeight / 2 + clamped.offsetY * exportScale
