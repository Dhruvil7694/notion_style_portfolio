...
import { actionError, type ActionResult } from "@/lib/admin/schemas"
import { revalidatePublicResume } from "@/lib/public/revalidate-cache"
...
export async function uploadProjectCoverImage(
  ...
  const filename = sanitizeFilename(file.name.replace(/\.[^.]+$/, "")) || "cover"
