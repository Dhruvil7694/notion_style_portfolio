import { normalizeSkillKey } from "@/lib/public/skills-showcase"
import type { Skill } from "@/types/database.helpers"

type SupabaseMutationClient = Awaited<
  ReturnType<typeof import("@/lib/admin/actions/client").getAdminMutationClient>
>
...
export async function syncSkillsFromTechStack(
  ...
  const normalizedStack = techStack
    .map((item) => item.trim())
    .filter(Boolean)
