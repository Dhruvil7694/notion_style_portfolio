...
import { faqItemSchema, projectFactsSchema } from "@/lib/knowledge/schemas"
import {
...
export const projectFormSchema = z
  .object({
    ...
    tradeoffs: z
      ...
      .transform((items) =>
        items.filter(
          (item) => item.decision.trim() && (item.reason?.trim() || item.tradeoff?.trim())
        )
      ),
    my_contribution: z
    ...
    expertise_slugs: z.array(z.string().trim()).default([]).transform((items) => items.filter(Boolean)),
    technologies: z.string().transform(parseCommaList),
