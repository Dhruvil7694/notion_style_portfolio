...
export async function getDashboardStats() {
  ...
  return {
    ...
    resume: {
      ...
      lastUploadDate: activeResume?.uploaded_at ?? resumes[0]?.uploaded_at ?? null,
    },
    errors: [
...
export async function getContentList(params: AdminListParams = {}) {
  ...
  let query = supabase
    .from("content")
    .select("id, title, type, status, published_at, updated_at", { count: "exact" })
    .order("updated_at", { ascending: false })
    .range(from, to)
...
export async function getExperienceList(params: AdminListParams = {}) {
  ...
  let query = supabase
    .from("experience")
    .select("id, company, role, start_date, end_date, display_order, updated_at", {
      count: "exact",
    })
    .order("display_order", { ascending: true })
    .range(from, to)
...
export async function getSkillsList(params: AdminListParams = {}) {
  ...
  let query = supabase
    .from("skills")
    .select("id, category, name, proficiency, display_order, show_on_landing", { count: "exact" })
    .order("display_order", { ascending: true })
    .range(from, to)
