...
export async function PublicLayout({ children }: PublicLayoutProps) {
  const [settings, resume] = await Promise.all([getPublicSettings(), getActiveResume()])
        ...
        <DiscoveryShell>
          <AssistantShell>
            <div className="public-site flex min-h-full flex-col">
              <ScrollProgress />
              <FloatingDock resumeAvailable={Boolean(resume?.file_path)} settings={settings} />
              <SiteHeader settings={settings} />
              <main className="dock-main flex-1">{children}</main>
              <ThankYouDivider />
              <SiteFooter resumeAvailable={Boolean(resume?.file_path)} settings={settings} />
            </div>
          </AssistantShell>
        </DiscoveryShell>
      </SmoothScrollProvider>
