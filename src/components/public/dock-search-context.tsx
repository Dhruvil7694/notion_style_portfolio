...
import { isOpenPaletteShortcut } from "@/lib/discovery/shortcuts"
...
export function DockSearchProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <DockSearchContext.Provider value={value}>{children}</DockSearchContext.Provider>
  )
}

export function useDockSearchRegistration(controller: DockSearchController): void {
