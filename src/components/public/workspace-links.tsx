...
import { getActiveSocialLinks } from "@/lib/public/social-links"
...
export function WorkspaceLinks({ settings, resumeAvailable }: WorkspaceLinksProps) {
  ...
  if (contact.email) {
    links.push({ key: "email", label: "Email", href: `mailto:${contact.email}` })
  }
    ...
    <ul className="workspace-links">
      {links.map((link, index) => (
        <li className="workspace-links-item" key={link.key}>
          {index > 0 ? <span aria-hidden className="workspace-links-sep">·</span> : null}
          {link.external ? (
            <a
