...
export function DiscoveryEntitySection({ title, items, className }: DiscoveryEntitySectionProps) {
  if (items.length === 0) {
    return null
  }
...
export function EntityNavigationSections({
  ...
  return (
    <>
      <DiscoveryEntitySection items={relatedExpertise} title="Related Expertise" />
      <DiscoveryEntitySection items={relatedTechnologies} title="Related Technologies" />
      <DiscoveryEntitySection items={relatedConcepts} title="Related Concepts" />
...
function flatIndexForGroup(groups: GroupedDiscoveryResults[], groupIndex: number, itemIndex: number) {
  let index = 0
              ...
              return (
                <li key={item.id} role="presentation">
                  <button
                    ...
                    className={cn("discovery-search-result", active && "discovery-search-result-active")}
...
export function DiscoverySuggestionList({ title, items, onSelect }: DiscoverySuggestionListProps) {
  if (items.length === 0) {
    return null
  }
