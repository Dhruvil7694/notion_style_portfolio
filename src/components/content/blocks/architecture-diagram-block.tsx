...
function layoutNodes(nodes: ArchitectureNode[]): Array<ArchitectureNode & { x: number; y: number }> {
...
export function ArchitectureDiagramBlock({
  ...
  const height = Math.max(...positioned.map((node) => node.y + NODE_HEIGHT), 120) + PADDING

  return (
    <figure
      aria-label="Architecture diagram"
      className={cn("bg-muted/20 border-border overflow-x-auto rounded-lg border p-4", className)}
    >
      <svg
