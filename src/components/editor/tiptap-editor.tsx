...
import { EditorStatus, type SaveState } from "@/components/editor/editor-status"
import { EditorToolbar } from "@/components/editor/editor-toolbar"
import { contentEditorExtensions } from "@/components/editor/extensions"
...
export function TiptapEditor({
  ...
  const handleEditorUpdate = useCallback(
    (editor: NonNullable<ReturnType<typeof useEditor>>) => {
      ...
      const matchedSlash = (Object.keys(SLASH_COMMANDS) as SlashCommandKey[]).find(
        (key) => parentText === key || parentText.startsWith(`${key} `)
      )
  ...
  useEffect(() => {
    ...
    if (!documentsEqual(value, toContentDocument(editor.getJSON()))) {
      editor.commands.setContent(fromContentDocument(value), { emitUpdate: false })
  ...
  const slashOptions = (Object.keys(SLASH_COMMANDS) as SlashCommandKey[]).filter(
    (key) => !slashQuery || key.startsWith(slashQuery)
  )
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      <p className="text-muted-foreground text-xs">
        Shortcuts: ⌘/Ctrl+B bold · ⌘/Ctrl+I italic · ⌘/Ctrl+Shift+7 numbered list ·
        ⌘/Ctrl+Shift+8 bullet list · Slash: /heading /quote /code /list /divider
        /callout /glossary /project /expandable /architecture /mention
      </p>
    </div>
  )
}
