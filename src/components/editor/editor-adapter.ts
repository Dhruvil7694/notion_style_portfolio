...
function extractListItems(listNode: JSONContent): string[] {
  ...
  for (const listItem of listNode.content ?? []) {
    ...
    const text = richTextToPlainText(richTextFromTiptap(listItem.content?.[0]?.content))
    if (text.trim()) {
...
export function validateEditorContent(json: JSONContent): EditorValidationResult {
  try {
    const document = toContentDocument(json)
    const parsed = contentDocumentSchema.safeParse(document)

    if (!parsed.success) {
      const message = parsed.error.issues[0]?.message ?? "Invalid editor content"
      return {
        success: false,
        error: message,
    ...
    return { success: true, data: serializeContent(normalizeContentDocument(parsed.data)) }
  } catch {
    return {
      success: false,
...
export function documentsEqual(a: ContentDocument, b: ContentDocument): boolean {
  return JSON.stringify(serializeContent(a)) === JSON.stringify(serializeContent(b))
}
        ...
        .insertContent({
          ...
          attrs: {
            ...
            contentJson: JSON.stringify([
              { type: "paragraph", content: [{ text: "Nested content goes here." }] },
            ]),
          },
        })
        .run()
