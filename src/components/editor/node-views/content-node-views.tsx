...
import { TextArea, TextInput } from "@/components/admin/forms/form-field"
import { ArchitectureDiagramBlock } from "@/components/content/blocks/architecture-diagram-block"
import { CalloutBlock } from "@/components/content/blocks/callout-block"
import { GlossaryTermBlock } from "@/components/content/blocks/glossary-term-block"
import { MentionBlock } from "@/components/content/blocks/mention-block"
import { ProjectReferenceBlock } from "@/components/content/blocks/project-reference-block"
import { ARCHITECTURE_PRESETS } from "@/lib/content/components/architecture-presets"
import type { InlineSpan } from "@/lib/content/inline"
...
function CalloutNodeView({ node, updateAttributes }: NodeViewProps) {
  const content = JSON.parse(String(node.attrs.contentJson ?? "[]")) as InlineSpan[]
  const variant = node.attrs.variant as "info" | "success" | "warning" | "danger"
...
function ProjectReferenceNodeView({ node, updateAttributes }: NodeViewProps) {
  return (
    <NodeShell label="Project Reference">
      <TextInput
        onChange={(event) => updateAttributes({ projectId: event.target.value })}
...
function ArchitectureDiagramNodeView({ node, updateAttributes }: NodeViewProps) {
  ...
  return (
    <NodeShell label="Architecture Diagram">
      <div className="mb-3 flex flex-wrap gap-2">
        {(Object.keys(ARCHITECTURE_PRESETS) as Array<keyof typeof ARCHITECTURE_PRESETS>).map(
          (key) => (
            <button
...
function ExpandableSectionNodeView({ node, updateAttributes }: NodeViewProps) {
  const nested = JSON.parse(String(node.attrs.contentJson ?? "[]")) as ContentBlock[]
  const nestedText =
