...
import { AI_FIRST_AUTOMATION_TOOLS } from "@/features/portfolio/lib/ai-first-content"
...
export function AiFirstAutomationToolsTable() {
  ...
  const pageCount = Math.max(
    1,
    Math.ceil(AI_FIRST_AUTOMATION_TOOLS.length / pageSize)
  )
  ...
  return (
    <section
      aria-labelledby="ai-first-tools-heading"
      className="ai-first-tools-table-section"
    >
      <header className="ai-first-tools-table-header">
        ...
        <p className="ai-first-tools-table-description">
          LLMs, agent skills, MCP servers, and custom pipelines — mapped to how
          I actually automate work.
        </p>
      </header>
      ...
      <div className="ai-first-tools-table-wrap">
        <table className="ai-first-tools-table">
          ...
          <tbody>
            {pageRows.map((row) => (
              <tr key={row.id}>
                <td className="ai-first-tools-table-category">
                  {row.category}
                </td>
                <td className="ai-first-tools-table-tool">
                  <span className="ai-first-tools-table-tool-cell">
                    <Icon
                      aria-hidden
                      className="ai-first-tools-table-icon"
                      icon={row.icon}
                    />
                    <span>{row.tool}</span>
                  </span>
                ...
                <td>{row.useFor}</td>
                <td className="ai-first-tools-table-notes">
                  {row.notes ?? "—"}
                </td>
              </tr>
            ))}
      ...
      <div className="ai-first-tools-table-pagination">
        ...
        <nav
          aria-label="Automation tools pagination"
          className="ai-first-tools-table-pager"
        >
          <button
