...
function ScoreBand({ score }: { score: number }) {
  const band = score >= 80 ? "Healthy" : score >= 50 ? "Warning" : "Critical"
  const color =
    score >= 80
      ...
      {failing.length > 0 ? (
        <div className="divide-y divide-white/5">
          {failing.map((check) => {
            ...
            const isSlug = check.ruleId === "slug_quality"
            return (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {check.suggestion ? (
                      <p className="mt-0.5 text-xs text-amber-600 dark:text-amber-400 line-clamp-2">
                        {check.suggestion}
                        {isSlug
                          ? " (changing slug breaks existing URLs — do carefully)"
                          : ""}
                      </p>
                    ) : null}
                    {isManual ? (
        ...
        <div className="px-3 py-3 text-sm text-green-600 dark:text-green-400 flex items-center gap-2">
          <CheckCircle2 className="size-4 shrink-0" aria-hidden />
          All SEO rules passing
        </div>
      )}
          ...
          {passingOpen ? (
            <div className="divide-y divide-white/5 pb-1">
              {passing.map((check) => (
                ...
                >
                  <div className="mt-0.5 size-4 shrink-0 rounded-full bg-green-500/15 flex items-center justify-center">
                    <CheckCircle2
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="shrink-0 text-xs tabular-nums text-green-600/70 dark:text-green-400/70">
                        {check.earned}pts
                      </span>
                    </div>
                    {check.currentValue ? (
                      <p className="mt-0.5 truncate font-mono text-sm text-muted-foreground/60">
                        {check.currentValue}
                      </p>
                    ) : null}
                  </div>
...
export function SeoAuditDrawer({
  ...
  >("idle")
  const [aiPreview, setAiPreview] = useState<SeoPreview | null>(null)
  const [editedFix, setEditedFix] = useState<EditableSeoFix | null>(null)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data = (await res.json()) as {
        ok?: boolean
        error?: string
        before?: SeoPreview["before"]
        after?: SeoPreview["after"]
        reasoning?: string
      }
      if (!res.ok || !data.ok) {
      ...
      >
        ...
        <div className="shrink-0 border-b border-white/10 bg-white/5 px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            <div className="flex min-w-0 flex-1 items-center gap-2.5 overflow-hidden">
              <span className="shrink-0 text-sm font-semibold">SEO</span>
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className="font-mono text-2xl font-bold tabular-nums">
              {aiPhase === "done" && newScore !== null ? newScore : item.score}
            </span>
        ...
        <ScrollArea active={open}>
          <div className="space-y-4 pt-4">
            {/* AI Fix card */}
            <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-3 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium">Fix all with AI</p>
                  {aiPhase === "idle" ? (
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Generates seo_title, description, ai_summary,
                      key_takeaways, FAQ + summary.
                    </p>
                  ) : aiPhase === "generating" ? (
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Generating…
                ...
                {aiPhase === "idle" ? (
                  <button
                    className="flex shrink-0 items-center gap-1.5 rounded-lg border border-green-500/30 bg-green-500/10 px-3 py-1.5 text-xs font-medium text-green-700 transition-colors hover:bg-green-500/20 dark:text-green-400"
                ...
                ) : aiPhase === "generating" ? null : aiPhase === "preview" ||
                  aiPhase === "applying" ? (
                  <button
              ...
              {aiPhase === "generating" ? (
                <div className="mt-3 space-y-1.5">
                  <TextShimmer className="text-sm" duration={2}>
                    Analyzing SEO signals and target keywords…
                  </TextShimmer>
                  <TextShimmer className="text-sm" duration={2.5} spread={15}>
                    Crafting title, description, AI summary, and FAQ…
                  </TextShimmer>
                </div>
              ) : null}

              {(aiPhase === "preview" || aiPhase === "applying") &&
              aiPreview &&
              editedFix ? (
                <div className="mt-3 space-y-4">
                  <div className="rounded-lg border border-white/10 bg-white/5 p-2.5">
                    <p className="text-sm font-medium">What AI improved</p>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {aiPreview.before.seo_title !== editedFix.seo_title &&
                    aiPreview.before.seo_title ? (
                    ...
                    <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-2.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditedFix(
                            (f) => f && { ...f, seo_title: e.target.value }
                          )
                        }
                        value={editedFix.seo_title}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {aiPreview.before.seo_description !==
                      editedFix.seo_description &&
                    aiPreview.before.seo_description ? (
                      <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-2.5">
                        <p className="mb-1 text-xs font-medium uppercase tracking-wide text-red-500/70">
                          Before
                    ...
                    <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-2.5">
                      <div className="mb-1 flex items-center justify-between">
                        ...
                        <span
                          className={cn(
                            "text-xs tabular-nums",
                            editedFix.seo_description.length > 160
                              ? "text-red-500"
                              : editedFix.seo_description.length >= 120
                                ? "text-green-600 dark:text-green-400"
                                : "text-muted-foreground"
                          )}
                        >
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditedFix(
                            (f) =>
                              f && { ...f, seo_description: e.target.value }
                          )
                        }
                        rows={3}
                        value={editedFix.seo_description}
                      />
                    </div>
                  </div>
                  ...
                  <SeoDiffField
                    ...
                    before={aiPreview.before.ai_summary}
                    label="AI Summary"
                    onEdit={(v) =>
                  ...
                  <SeoDiffField
                    ...
                    before={aiPreview.before.summary}
                  ...
                  />
                  <SeoListDiffField
                    before={aiPreview.before.key_takeaways}
                  ...
                  />
                  <SeoListDiffField
                    before={aiPreview.before.tags}
                    items={editedFix.tags}
                    label="Tags"
                    onEdit={(v) => setEditedFix((f) => f && { ...f, tags: v })}
                  />
                  <SeoFaqDiffField
                    before={aiPreview.before.faq}
                    items={editedFix.faq}
                    label="FAQ"
                  ...
                  {aiPhase === "applying" ? (
                    <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4">
                      <div className="flex items-center gap-3">
                        <span className="size-4 shrink-0 animate-spin rounded-full border-2 border-green-500/30 border-t-green-500" />
                    ...
                    <button
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-green-500/40 bg-green-500/15 py-3 text-sm font-semibold text-green-700 transition-all hover:bg-green-500/25 hover:shadow-[0_0_12px_rgba(34,197,94,0.15)] active:scale-[0.99] dark:text-green-300"
              ...
              {aiPhase === "done" ? (
                <div className="mt-3 rounded-lg border border-green-500/25 bg-green-500/10 p-3">
            ...
            {item.issueCount === 0 && item.score < 100 ? (
              <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-3 backdrop-blur-sm">
                <p className="text-xs font-medium text-green-600 dark:text-green-400">
                  All SEO rules pass — score below 100
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  SEO score includes content-health fields (overview, problem,
                  results, expertise links) worth ~29pts. All SEO rules passing
                  gives max 71/100. Improve content depth and AEO/GEO signals to
                  reach 100.
                </p>
                <div className="mt-2.5 flex flex-wrap gap-2">
                  <Link
                    className="flex items-center gap-1 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600 transition-colors hover:bg-emerald-500/20 dark:text-emerald-400"
                    href={`/admin/aeo?highlight=${item.id}`}
                  >
...
function SeoDiffField({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-2.5">
        <p className="mb-1 text-xs font-medium uppercase tracking-wide text-green-600/70">
...
function SeoListDiffField({
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-sm font-medium">{label}</p>
      <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-2.5">
        <p className="mb-2 text-xs font-medium uppercase tracking-wide text-green-600/70">
          After (editable)
        </p>
        {items.map((item, idx) => {
            ...
            <div key={idx} className="flex items-start gap-2 py-1">
              <span
                className={cn(
                  ...
                  isNew ? "bg-green-500" : "bg-white/20"
                )}
              />
              <input
            ...
            >
              <div className="flex items-start justify-between gap-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {isNew ? (
                    <span className="rounded-full bg-green-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-green-600 transition-opacity duration-700 dark:text-green-400">
                      NEW
                    </span>
                  ) : null}
                </div>
