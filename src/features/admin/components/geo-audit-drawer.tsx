...
import type { GeoAuditScore } from "@/features/geo/lib/audit/types"
...
const MANUAL_RULES = new Set([
  "entity_title_clarity",
  "technology_tags",
  "content_depth_overview",
  "content_depth_approach",
  "content_depth_results",
  "unique_facts",
])
...
function ScoreBand({ score }: { score: number }) {
  const band = score >= 70 ? "Prominent" : score >= 35 ? "Emerging" : "Absent"
  const color =
    score >= 70
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/8">
        <p className="text-sm font-medium">GEO checklist</p>
      ...
      {failing.length > 0 ? (
        ...
        <div className="px-3 py-3 text-sm text-blue-600 dark:text-blue-400 flex items-center gap-2">
          <CheckCircle2 className="size-4 shrink-0" aria-hidden />
          All GEO rules passing
        </div>
      )}
          ...
          {passingOpen ? (
            <div className="divide-y divide-white/5 pb-1">
              {passing.map((check) => (
                ...
                >
                  <div className="mt-0.5 size-4 shrink-0 rounded-full bg-blue-500/15 flex items-center justify-center">
                    <CheckCircle2
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="shrink-0 text-xs tabular-nums text-blue-600/70 dark:text-blue-400/70">
                        {check.earned}pts
                      </span>
                    </div>
                    {check.currentValue ? (
...
type Props = {
  item: GeoAuditScore | null
  allItems: GeoAuditScore[]
  open: boolean
  onOpenChange: (v: boolean) => void
  onSelectItem: (item: GeoAuditScore) => void
  onApplied?: (id: string) => void
}

export function GeoAuditDrawer({
  item,
  allItems,
  open,
  onOpenChange,
  ...
  >("idle")
  const [previewData, setPreviewData] = useState<GeoPreview | null>(null)
  const [editedFix, setEditedFix] = useState<EditableGeoFix | null>(null)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data = (await res.json()) as {
        ok?: boolean
        error?: string
        before?: GeoPreview["before"]
        after?: GeoPreview["after"]
        reasoning?: string
      }
      if (!res.ok || !data.ok) {
    ...
    <Sheet onOpenChange={onOpenChange} open={open}>
      ...
      >
        <div className="shrink-0 border-b border-white/10 bg-white/5 px-4 py-3">
          <SheetTitle className="sr-only">GEO Audit</SheetTitle>
          <div className="flex items-center gap-3">
            <div className="flex min-w-0 flex-1 items-center gap-2.5 overflow-hidden">
              <span className="shrink-0 text-sm font-semibold">GEO</span>
        ...
        <ScrollArea active={open}>
          <div className="space-y-4 pt-4">
            {/* AI Fix card */}
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium">Fix all with AI</p>
                  {phase === "idle" ? (
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Generates ai_summary, key_takeaways, concepts, FAQ,
                      summary, tags + expertise links.
                    </p>
                  ) : phase === "generating" ? (
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Generating…
                ...
                {phase === "idle" ? (
                  <button
                    className="flex shrink-0 items-center gap-1.5 rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-1.5 text-xs font-medium text-blue-700 transition-colors hover:bg-blue-500/20 dark:text-blue-400"
              ...
              {phase === "generating" ? (
                <div className="mt-3 space-y-1.5">
                  <TextShimmer className="text-sm" duration={2}>
                    Analyzing entity signals and concept coverage…
                  </TextShimmer>
                  <TextShimmer className="text-sm" duration={2.5} spread={15}>
                    Rewriting ai_summary for LLM entity recognition…
                  </TextShimmer>
                  <TextShimmer className="text-sm opacity-70" duration={3}>
                    Generating concepts, FAQ, and expertise links…
                  </TextShimmer>
                </div>
              ) : null}
              ...
              editedFix ? (
                <div className="mt-3 space-y-4">
                  <div className="rounded-lg border border-blue-500/15 bg-blue-500/5 p-2.5">
                  ...
                  />
                  <GeoListDiffField
                    before={previewData.before.tags}
                  ...
                  {phase === "applying" ? (
                    <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
                      <div className="flex items-center gap-3">
                        <span className="size-4 shrink-0 animate-spin rounded-full border-2 border-blue-500/30 border-t-blue-500" />
                    ...
                    <button
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-blue-500/40 bg-blue-500/15 py-3 text-sm font-semibold text-blue-700 transition-all hover:bg-blue-500/25 hover:shadow-[0_0_12px_rgba(59,130,246,0.15)] active:scale-[0.99] dark:text-blue-300"
              ...
              {phase === "done" ? (
                <div className="mt-3 rounded-lg border border-blue-500/25 bg-blue-500/10 p-3">
            ...
            {item.issueCount === 0 && item.score < 100 ? (
              <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 backdrop-blur-sm">
                <p className="text-xs font-medium text-blue-600 dark:text-blue-400">
                  All GEO rules pass — boost other signals
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Near-threshold values cause partial credit. Also improve AEO
                  and SEO signals.
                </p>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-2.5">
        <p className="mb-1 text-xs font-medium uppercase tracking-wide text-blue-600/70 dark:text-blue-400/70">
...
function GeoListDiffField({
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-sm font-medium">{label}</p>
      <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-2.5">
        <p className="mb-2 text-xs font-medium uppercase tracking-wide text-blue-600/70 dark:text-blue-400/70">
          After (editable)
        </p>
        {items.map((item, idx) => {
            ...
            >
              <div className="flex items-start justify-between gap-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {isNew ? (
                    <span className="rounded-full bg-blue-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-blue-600 transition-opacity duration-700 dark:text-blue-400">
                      NEW
                    </span>
                  ) : null}
                </div>
