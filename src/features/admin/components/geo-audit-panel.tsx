...
import type {
  ...
  GeoHealthBand,
  GeoItemType,
} from "@/features/geo/lib/audit/types"
import { cn } from "@/shared/lib/utils"
import { Button } from "@/shared/ui/button"

import { GeoAuditDrawer } from "./geo-audit-drawer"

const PAGE_SIZE = 20

function ScoreBar({ score }: { score: number }) {
  const color =
    score >= 70 ? "bg-blue-500" : score >= 35 ? "bg-amber-500" : "bg-red-500"
  return (
...
function BandBadge({ band }: { band: GeoHealthBand }) {
  return (
    <span
      className={cn(
        ...
        band === "absent" && "bg-red-500/15 text-red-600 dark:text-red-400"
      )}
    >
...
function exportCsv(items: GeoAuditScore[]) {
  const header = "Title,Type,Score,Band,Issues"
  ...
  a.href = url
  a.download = `geo-audit-${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
...
type Props = { result: GeoAuditResult; highlightId?: string }

export function GeoAuditPanel({ result, highlightId }: Props) {
  const [typeFilter, setTypeFilter] = useState<GeoItemType | "all">("all")
  const [bandFilter, setBandFilter] = useState<GeoHealthBand | "all">("all")
  ...
  function openItem(item: GeoAuditScore) {
    ...
    <>
      <div className="flex flex-wrap items-center gap-2">
        <select
          ...
          onChange={(e) => {
            setTypeFilter(e.target.value as GeoItemType | "all")
            setPage(1)
          }}
          value={typeFilter}
        >
        ...
        <select
          className={SELECT_CLASS}
          onChange={(e) => {
            setBandFilter(e.target.value as GeoHealthBand | "all")
            setPage(1)
          }}
          value={bandFilter}
        >
                  ...
                  <td className="px-4 py-3 font-medium">
                    <span className="flex items-center gap-2">
                      ...
                      {recentlyFixed.has(item.id) ? (
                        <span className="rounded-full bg-blue-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-blue-600 dark:text-blue-400">
                          NEW
                        </span>
                      ) : null}
                    </span>
