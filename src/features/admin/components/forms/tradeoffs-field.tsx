...
import type { ProjectTradeoff } from "@/features/portfolio/lib/project-case-study"
...
export function TradeoffsField({
  value,
  onChange,
  error,
}: TradeoffsFieldProps) {
    ...
    <div className="space-y-5">
      {value.map((item, index) => (
        ...
        >
          ...
          <TextInput
            onChange={(event) =>
              updateItem(index, "decision", event.target.value)
            }
          ...
          <TextInput
            onChange={(event) =>
              updateItem(index, "alternative", event.target.value)
            }
            placeholder="CrewAI"
      ...
      <button
        ...
        onClick={() =>
          onChange([...value, { decision: "", alternative: "", reason: "" }])
        }
        type="button"
      >
