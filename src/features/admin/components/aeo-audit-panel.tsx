...
import type {
  ...
  AeoHealthBand,
  AeoItemType,
} from "@/features/aeo/lib/audit/types"
import { cn } from "@/shared/lib/utils"
import { Button } from "@/shared/ui/button"

import { AeoAuditDrawer } from "./aeo-audit-drawer"
...
function BandBadge({ band }: { band: AeoHealthBand }) {
  return (
    <span
      className={cn(
        ...
        band === "missing" && "bg-red-500/15 text-red-600 dark:text-red-400"
      )}
    >
...
function exportCsv(items: AeoAuditScore[]) {
  ...
  a.href = url
  a.download = `aeo-audit-${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
...
type Props = { result: AeoAuditResult; highlightId?: string }

export function AeoAuditPanel({ result, highlightId }: Props) {
  const [typeFilter, setTypeFilter] = useState<AeoItemType | "all">("all")
  const [bandFilter, setBandFilter] = useState<AeoHealthBand | "all">("all")
  ...
  function openItem(item: AeoAuditScore) {
    ...
    <>
      <div className="flex flex-wrap items-center gap-2">
        <select
          ...
          onChange={(e) => {
            setTypeFilter(e.target.value as AeoItemType | "all")
            setPage(1)
          }}
          value={typeFilter}
        >
        ...
        <select
          className={SELECT_CLASS}
          onChange={(e) => {
            setBandFilter(e.target.value as AeoHealthBand | "all")
            setPage(1)
          }}
          value={bandFilter}
        >
                  ...
                  <td className="px-4 py-3 font-medium">
                    <span className="flex items-center gap-2">
                      ...
                      {recentlyFixed.has(item.id) ? (
                        <span className="rounded-full bg-emerald-500/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-600 dark:text-emerald-400">
                          NEW
                        </span>
                      ) : null}
                    </span>
