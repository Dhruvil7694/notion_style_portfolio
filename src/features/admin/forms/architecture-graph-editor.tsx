...
import { LucideIconPicker } from "@/features/admin/forms/lucide-icon-picker"
import type {
  ArchitectureGraph,
...
} from "@/lib/diagrams/architecture-node-types"
import {
...
function GraphEditorCanvas({
  ...
  useEffect(() => {
    setFlowNodes(toReactFlowNodes(value.nodes, { draggable: true, selectable: true }))
    setFlowEdges(toReactFlowEdges(value.edges, value.nodes, { labelMode: "all" }))
  ...
  const emitChange = useCallback(
    (nextNodes: ArchitectureGraphNode[], nextEdges: ArchitectureGraphEdge[]) => {
  ...
  function deleteSelected() {
    if (selection.edgeId) {
      const nextEdges = value.edges.filter((edge) => edge.id !== selection.edgeId)
    ...
    if (selection.nodeId) {
      const nextNodes = value.nodes.filter((node) => node.id !== selection.nodeId)
      const nextEdges = value.edges.filter(
        (edge) => edge.source !== selection.nodeId && edge.target !== selection.nodeId
      )
  ...
  const selectedNode = value.nodes.find((node) => node.id === selection.nodeId) ?? null
  const selectedEdge = value.edges.find((edge) => edge.id === selection.edgeId) ?? null
        ...
        <aside className="border-border space-y-4 rounded-lg border p-4">
          ...
          {selectedNode ? (
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <LucideIconPicker
                  ...
                  value={selectedNode.icon || defaultIconForNodeType(selectedNode.type)}
                />
              </div>
            </div>
          ) : selectedEdge ? (
            ...
            <p className="text-muted-foreground text-sm">
              Select a node or edge to edit properties. Drag from the bottom handle of one
              node to the top handle of another to connect them.
            </p>
          )}
        </aside>
      </div>
