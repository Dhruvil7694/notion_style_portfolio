...
export function TimelineField({ value, onChange, error }: TimelineFieldProps) {
  function updateItem(index: number, field: keyof ProjectTimelineEntry, next: string) {
    onChange(
  ...
  return (
    <div className="space-y-4">
      {value.map((item, index) => (
        <div className="space-y-3 rounded-lg border border-border p-4" key={`timeline-${index}`}>
