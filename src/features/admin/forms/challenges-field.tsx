...
export function ChallengesField({ value, onChange, error }: ChallengesFieldProps) {
  function updateItem(index: number, field: keyof ProjectChallenge, next: string) {
    onChange(
    ...
    <div className="space-y-4">
      {value.map((item, index) => (
        ...
        >
          ...
          <TextInput
            onChange={(event) => updateItem(index, "challenge", event.target.value)}
            placeholder="What was the engineering challenge?"
            value={item.challenge}
          />
          <TextArea
            onChange={(event) => updateItem(index, "solution", event.target.value)}
