...
function convertLegacyTiptapDoc(raw: {
  type: "doc"
  content?: unknown[]
}): ContentBlock[] {
...
function sanitizeBlock(block: ContentBlock): ContentBlock | null {
  switch (block.type) {
    ...
    case "project_reference":
      return z.string().uuid().safeParse(block.project_id).success
        ? block
        : null
    case "mention":
...
export function safeParseContent(raw: unknown): ContentParseResult {
  ...
  try {
    ...
    const message =
      parsed.error.issues[0]?.message ?? "Invalid content structure"
