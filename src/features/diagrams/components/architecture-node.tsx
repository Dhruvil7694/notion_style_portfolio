...
import { resolveLucideIcon } from "@/features/diagrams/lib/lucide-icons"
    ...
    >
      ...
      <div className="architecture-graph-node-header">
        ...
        <span className="architecture-graph-node-type">
          {nodeData.nodeType.charAt(0).toUpperCase() +
            nodeData.nodeType.slice(1)}
        </span>
      </div>
      <p className="architecture-graph-node-label">{nodeData.label}</p>
      {!compact && nodeData.description?.trim() ? (
        <p className="architecture-graph-node-description">
