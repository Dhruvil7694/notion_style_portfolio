...
export function auditAeo(project: ProjectAuditInput): DimensionScore {
  ...
  const checks = [
    ...
    {
      label: "Question-ready summary",
      passed: hasText(project.summary) && faqCount > 0,
    },
  ]
...
export function auditDiscovery(project: ProjectAuditInput): DimensionScore {
  const checks = [
    ...
    {
      label: "Tagline or impact",
      passed: hasText(project.overview) || hasText(project.problem),
    },
...
export function auditCaseStudy(project: ProjectAuditInput): DimensionScore {
  const checks = [
    ...
    {
      label: "Approach",
      passed: hasArray(project.approach as unknown[] | null),
    },
    {
      label: "Architecture",
      passed:
    ...
    {
      label: "Tradeoffs",
      passed: Array.isArray(project.tradeoffs) && project.tradeoffs.length > 0,
    },
...
export function auditProjectAdvanced(
  project: ProjectAuditInput
): AdvancedProjectAudit {
  const dimensions = [
    auditSeo(project),
    auditAeo(project),
    auditGeo(project),
