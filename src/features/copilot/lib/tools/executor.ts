...
import { searchDocuments } from "@/features/discovery/lib/search"
import { getPublicSettings } from "@/features/portfolio/lib/queries"
import { revalidateKnowledgeAndDiscovery } from "@/features/portfolio/lib/revalidate-cache"
...
function isAboutField(value: unknown): value is AboutField {
  return (
    typeof value === "string" &&
    (ABOUT_FIELDS as readonly string[]).includes(value)
  )
}
...
async function generateAboutVariants(
  ...
  const settled = await Promise.all(
    ABOUT_VARIANT_TONES.map(
      async (tone): Promise<{ label: string; text: string } | null> => {
...
Return ONLY the rewritten paragraph. No quotes, preamble, label, or explanation.`
  ...
  return settled.filter((v): v is { label: string; text: string } => v !== null)
}
  ...
  try {
    switch (name) {
      ...
      case "auditProject": {
        ...
        if (!project)
          return { success: false, error: `Project not found: ${slug}` }
        return {
          success: true,
          data: {
      ...
      case "updateAboutSection": {
        ...
        const variantsArg = Array.isArray(args.variants)
          ...
          : null
        const variantIndex =
          typeof args.variantIndex === "number" &&
...
async function generateContentPreview(
  ...
  if (entityType === "project") {
    const { data } = await supabase
      .from("projects")
      .select(
        "title, summary, overview, problem, why_built, tech_stack, ai_summary"
      )
      .eq("slug", slug)
      .maybeSingle()
    entityData = data
