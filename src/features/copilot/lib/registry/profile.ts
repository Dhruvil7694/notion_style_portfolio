...
import { revalidatePublicLayoutData } from "@/features/portfolio/lib/revalidate-cache"
  ...
  async build(rawArgs) {
    ...
    return {
      ...
      pending: {
        ...
        fields: [
          { name: field, key: field, before: currentValue, after: value },
        ],
...
const proposeSocialLinkSchema = z.object({
  ...
  url: z.string().nullable().describe("Full URL or null to remove the link."),
})
  ...
  async build(rawArgs) {
    ...
    return {
      ...
      pending: {
        ...
        fields: [
          { name: platform, before: current[platform] ?? null, after: url },
        ],
...
export const applySocialLinkTool: CopilotApplyTool<
  typeof proposeSocialLinkSchema
> = {
  kind: "apply",
...
> = {
  ...
  description:
    "Propose updating a contact info field (email, location, calendly_url).",
...
export const applyContactInfoTool: CopilotApplyTool<
  typeof proposeContactInfoSchema
> = {
  kind: "apply",
