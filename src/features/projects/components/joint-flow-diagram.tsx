...
import {
  ...
  JOINT_FLOW_LAYOUT,
} from "@/features/diagrams/lib/build-linear-flow-graph"
...
export function JointFlowDiagram({
  ...
  const labels = useMemo(
    () => nodes.map((node) => node.trim()).filter(Boolean),
    [nodes]
  )
  ...
  if (!mounted) {
    return (
      <div
        aria-hidden
        className={cn(
          "joint-flow-diagram joint-flow-diagram-loading",
          className
        )}
        style={{
          height: graph.height,
          minHeight: JOINT_FLOW_LAYOUT.nodeHeight,
        }}
      />
    )
  }
