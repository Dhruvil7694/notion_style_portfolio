...
export function formatExperienceDateLabel(value: string): string {
  return formatDate(value, "dd MMM yyyy").toUpperCase()
}
...
export function formatExperienceDateRangeLabel(
  ...
  const startLabel = formatExperienceDateLabel(startDate)
  const endLabel = endDate ? formatExperienceDateLabel(endDate) : "Present"
