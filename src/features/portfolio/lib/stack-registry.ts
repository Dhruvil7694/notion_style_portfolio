import { inferSkillCategory } from "@/features/admin/lib/sync-skills-from-tech-stack"
import {
...
export type StackProjectSource = Pick<
  Project,
  "id" | "slug" | "title" | "tech_stack"
> &
...
function registerTechnologyName(names: Map<string, string>, value: string) {
...
export function buildStackShowcaseGroups(
  ...
  const buildGroups = (landingOnly: boolean) => {
    ...
    const buckets = Object.fromEntries(
      SKILLS_SHOWCASE_ORDER.map((category) => [
        category,
        [] as StackShowcaseGroup["items"],
      ])
