...
const STACK_CATEGORIES: Array<{
  ...
  },
  {
    label: "Cloud",
    matcher: (item) =>
      /(^|\b)(aws|azure|gcp|sagemaker|ec2|cloud|google drive|drive api)(\b|$)/.test(
        item
      ),
  },
  {
  ...
  },
  {
    label: "Frontend",
    matcher: (item) =>
      /(^|\b)(react|next|typescript|javascript|frontend|ui|tailwind|html|css)(\b|$)/.test(
        item
      ),
  },
  {
...
const INFERRED_TECH: Array<{ pattern: RegExp; label: string }> = [
  ...
  {
    pattern: /\bhybrid\b.*\brag\b|\brag\b.*\bhybrid\b/i,
    label: "Hybrid Extraction",
  },
...
export function groupExperienceTechStack(
  stack: string[]
): ExperienceStackGroups {
...
export function resolveTechStackIcon(label: string): string {
  ...
  const partial = Object.entries(TECH_ICON_MAP).find(([name]) =>
    key.includes(name)
  )
  if (partial) {
    return partial[1]
