...
const TIMEBLOCK_POOLS: Partial<Record<WorkspaceState, ContextMessage[]>> = {
  ...
  researching: weighted([
    ["Research session", 2],
    ["Evaluating systems", 2],
  ]),
  building: weighted([
    ["Build session", 2],
    ["In production mode", 2],
  ]),
  shipping: weighted([
    ["Still shipping", 3],
    ["Evening deploy window", 2],
  ]),
  learning: weighted([
    ["Learning block", 2],
    ["Reading tonight", 1],
...
const NAME_POOLS: Partial<Record<WorkspaceState, ContextMessage[]>> = {
  sleeping: weighted([
    ["Probably sleeping", 1],
    ["Offline for the night", 1],
  ]),
  planning: weighted([
    ["Planning the week", 2],
    ["Coffee first, then code", 1],
  ]),
  deep_work: weighted([
    ["Building production AI systems", 3],
...
export function getAvatarHoverMessagePool(
  context: WorkspaceContext
): ContextMessage[] {
