...
const STATIC_ROUTES = [
  ...
  { path: "/stack", changeFrequency: "monthly" as const, priority: 0.65 },
...
export async function buildSitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const expertiseEntries: MetadataRoute.Sitemap = (expertiseAreas ?? []).map(
    (area) => ({
  ...
  const technologyEntries: MetadataRoute.Sitemap = (
    graph?.technologies ?? []
  ).map((tech) => ({
  ...
  const conceptEntries: MetadataRoute.Sitemap = (graph?.concepts ?? []).map(
    (concept) => ({
