...
import { collectStackTechnologyNames } from "@/features/portfolio/lib/stack-registry"
...
async function buildKnowledgeGraphUncached(
  ...
  const registryBySlug = new Map(
    (technologyRegistry ?? []).map((item) => [
      normalizeTechnologySlug(item.slug),
      item,
    ])
  )
  const conceptRegistryBySlug = new Map(
    (conceptRegistry ?? []).map((item) => [
      normalizeConceptSlug(item.slug),
      item,
    ])
  )
  ...
  for (const project of projectRows) {
    ...
    relationships.push(
      ...relationsForExpertiseLinks(entity.id, meta.expertiseSlugs)
    )
    relationships.push(
      ...relationsForTechnologyLinks(entity.id, extracted.technologies)
    )
    relationships.push(
      ...relationsForConceptLinks(entity.id, extracted.concepts)
    )
  ...
  const technologies = [...technologySlugs]
    ...
    .map((slug) => {
      ...
      entities.push(technologyToEntity(slug, siteUrl, record ?? undefined))
      return {
        slug,
...
export function findRelatedKnowledge(
  ...
  const byType = (type: KnowledgeEntity["type"]) =>
    graph.entities.filter(
      (entity) => entity.type === type && entity.id !== source.id
    )
  ...
  const expertiseEntities = (source.expertiseSlugs ?? [])
    .map((slug) =>
      graph.entities.find((entity) => entity.id === `expertise:${slug}`)
    )
    .filter(Boolean) as KnowledgeEntity[]

  const technologyEntities = (source.technologySlugs ?? [])
    .map((slug) =>
      graph.entities.find((entity) => entity.id === `technology:${slug}`)
    )
    .filter(Boolean) as KnowledgeEntity[]
  ...
  const conceptEntities = (source.concepts ?? [])
    .map((concept) =>
      graph.entities.find(
        (entity) => entity.id === `concept:${normalizeConceptSlug(concept)}`
      )
    )
    .filter(Boolean) as KnowledgeEntity[]
...
export function getExpertiseBundle(
  ...
  const area =
    graph.expertise.find((item) => item.slug === expertiseSlug) ?? null
  const linkedIds = new Set(
    graph.relationships
      .filter(
        (rel) =>
          rel.type === "uses_expertise" &&
          rel.targetId === `expertise:${expertiseSlug}`
      )
...
export function getTechnologyBundle(
  ...
  const technology =
    graph.entities.find((entity) => entity.id === `technology:${normalized}`) ??
    null
...
export function getConceptBundle(
  ...
  const concept =
    graph.entities.find((entity) => entity.id === `concept:${normalized}`) ??
    null
  const registry =
    graph.concepts.find((item) => item.slug === normalized) ?? null
  ...
  const relatedConcepts = relatedConceptSlugs
    .map((slug) =>
      graph.entities.find((entity) => entity.id === `concept:${slug}`)
    )
    .filter(Boolean) as KnowledgeEntity[]

  const conceptRecord = graph.entities.find(
    (entity) => entity.id === `concept:${normalized}`
  )
  const relatedExpertiseSlugs = Array.isArray(
    conceptRecord?.metadata?.relatedExpertiseSlugs
  )
  ...
  const linkedExpertise = graph.expertise
    ...
    .map((item) =>
      graph.entities.find((entity) => entity.id === `expertise:${item.slug}`)
    )
    .filter(Boolean) as KnowledgeEntity[]
  ...
  return {
    ...
    relatedExpertise: [...explicitExpertise, ...linkedExpertise].filter(
      (entity, index, list) =>
        list.findIndex((item) => item.id === entity.id) === index
    ),
    projects: linked.filter((entity) => entity.type === "project"),
    research: linked.filter((entity) => entity.type === "research"),
    writing: linked.filter((entity) => entity.type === "writing"),
...
export function linkContentByExpertiseSlug<
  T extends { expertise_slugs?: string[] | null },
>(items: T[], expertiseSlug: string): T[] {
  return items.filter((item) =>
    (item.expertise_slugs ?? []).includes(expertiseSlug)
  )
}

export function linkProjectsByTechnology<
  T extends { tech_stack?: string[] | null; technologies?: string[] | null },
>(items: T[], technologySlug: string): T[] {
  const normalized = normalizeTechnologySlug(technologySlug)
