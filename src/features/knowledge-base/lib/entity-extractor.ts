import {
  ...
  TECHNOLOGY_ALIASES,
} from "@/features/knowledge-base/lib/taxonomy"

const TECHNOLOGY_PATTERNS = Object.entries(TECHNOLOGY_ALIASES).map(
  ([slug, label]) => ({
    slug,
    label,
...
export function extractEntitiesFromText(
  ...
  const technologies = TECHNOLOGY_PATTERNS.filter(({ pattern }) =>
    pattern.test(haystack)
  ).map(({ slug }) => slug)

  const concepts = CONCEPT_PATTERNS.filter(({ pattern }) =>
    pattern.test(haystack)
  ).map(({ concept }) => concept)
...
export function extractEntitiesFromProject(input: {
  ...
  const fromStack = (input.tech_stack ?? []).map((tech) =>
    normalizeTechnologySlug(tech)
  )
  const explicitTech = (input.technologies ?? []).map((tech) =>
    normalizeTechnologySlug(tech)
  )

  return {
    technologies: uniqueSorted([
      ...fromText.technologies,
      ...fromStack,
      ...explicitTech,
    ]),
